program test_im_int_array
  ...
  type(c_ptr) :: im, inpipe, outpipe, pngout
  ...
  integer(c_int)    :: workmatrix_linear(3*w*h)

  integer(c_int)   :: yellow, greyvalue, x_counter, y_counter
  integer(c_int)   :: r, g, b
  ...
  do while(.true.)
     ...
     do y_counter=0,gdImageSy(im)-1
        do x_counter=0,gdImageSx(im)-1
           greyvalue = sum (workmatrix(:, x_counter, y_counter))/3
